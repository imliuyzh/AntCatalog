...
describe('test determineIndexFilePath()', () => {
    test('should respond with correct path when PRODUCTION_ENV is aws', async () => {
        process.env.PRODUCTION_ENV = 'aws';
        expect(determineIndexFilePath()).toContain('client/build/index.html');
    });
    test('should respond with correct path when PRODUCTION_ENV is azure', async () => {
        process.env.PRODUCTION_ENV = 'azure';
        expect(determineIndexFilePath()).toContain('public/index.html');
    });
    test('should throw exception when PRODUCTION_ENV is something else', async () => {
...
describe('test determineStaticFileFolder()', () => {
    test('should respond with correct folder when PRODUCTION_ENV is aws', async () => {
        process.env.PRODUCTION_ENV = 'aws';
        expect(determineStaticFileFolder()).toContain('client/build');
    });
    test('should respond with correct folder when PRODUCTION_ENV is azure', async () => {
        process.env.PRODUCTION_ENV = 'azure';

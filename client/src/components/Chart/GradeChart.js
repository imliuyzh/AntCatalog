		...
		const config = {
			...
			tooltip: {
				customContent: (title, items) =>
					<>
						{ (searchResultState.isAggregateData) ? '' : <h5 style={{ marginTop: 16 }}>{title}</h5> }
							...
							{items?.map(({ data, color }) => 
								...
								>
									<div style={{
										...
										<span>{(searchResultState.isAggregateData) ? data.grade : data.title}</span>
									</div>
									    {
									        (searchResultState.isAggregateData)
									            ?
